...
#define ngx_js_http_error(http, err, fmt, ...)                                \
    (http)->error_handler(http, err, fmt, ##__VA_ARGS__)
...
ngx_js_http_process_status_line(ngx_js_http_t *http)
{
    ...
    ngx_js_http_error(http, 0, "invalid http status line");
...
ngx_js_http_process_headers(ngx_js_http_t *http)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_OK) {
            ...
            ret = http->headers_append(http, &http->response.headers,
                                   ...
                                   (u_char *) "Content-Length", len) == 0)
            {
                ...
                if (hp->content_length_n == NGX_ERROR) {
                    ngx_js_http_error(http, 0, "invalid http content length");
                    return NGX_ERROR;
                }
        ...
        ngx_js_http_error(http, 0, "invalid http header");
...
ngx_js_http_process_body(ngx_js_http_t *http)
{
    ...
    if (http->done) {
        ...
        {
            ngx_js_http_error(http, 0, "invalid http chunked response");
            return NGX_ERROR;
        }
        ...
        ngx_js_http_error(http, 0, "http trailing data");
        return NGX_ERROR;
    }
    ...
    if (http->http_parse.chunked) {
        ...
        if (size > http->max_response_body_size * 10) {
            ngx_js_http_error(http, 0, "very large http chunked response");
            return NGX_ERROR;
        }
        ...
        if (size + chsize > http->max_response_body_size) {
            ngx_js_http_error(http, 0, "http response body is too large");
            return NGX_ERROR;
        }
...
void
ngx_js_http_done(ngx_js_http_t *http)
{
    if (http->ctx != NULL) {
